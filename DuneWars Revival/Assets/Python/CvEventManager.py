...
# M -> changes in the hard code, many things are not reported here
...
#################### ON EVENTS ######################
	...
	def onGameStart(self, argsList):
		...
		mapScriptName = CyMap().getMapScriptName()
		CvUtil.pyPrint('EM:mapScriptName: %s' %(mapScriptName))
		if '.CivBeyondSwordWBSave' in mapScriptName or '.Civ4WorldBuilderSave' in mapScriptName:
			mapScriptNameNoExt = mapScriptName.split(".")[0]
			CvUtil.pyPrint('EM:mapScriptNameNoExt: %s' %(mapScriptNameNoExt))
			if "Generate" in mapScriptNameNoExt:
				generate_code = mapScriptNameNoExt.split("Generate", 1)[1]
				CvUtil.pyPrint('EM:generate_code: %s' %(generate_code))
				if "F" in generate_code:
					CyMapGenerator().eraseFeatures()
				if "B" in generate_code:
					CyMapGenerator().eraseBonuses()
				if "G" in generate_code:
		...
		if (gc.getGame().getGameTurnYear() == gc.getDefineINT("START_YEAR") and not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_ADVANCED_START)):
			for iPlayer in xrange(gc.getMAX_PLAYERS()):
				...
				if (player.isAlive() and player.isHuman()):
                                        # !M! -> intro movie
                                        if not CyGame().isGameMultiPlayer() and not CyGame().isPitbossHost():
                                                if iPlayer == CyGame().getActivePlayer() and not CyUserProfile().getGraphicOption(GraphicOptionTypes.GRAPHICOPTION_NO_MOVIES):
                                                        if CyGame().isFinalInitialized() and not CyGame().GetWorldBuilderMode():
                                                                popupInfo = CyPopupInfo()
                                                                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
                                                                popupInfo.setData1(-1) # iType
                                                                popupInfo.setData2(2) # iMovieEventId
		...
		if gc.getGame().getHandicapType() >= 5:
			for iPlayer in xrange(gc.getMAX_CIV_PLAYERS()):
				player = gc.getPlayer(iPlayer)
				if (player.isAlive() and not player.isHuman()):
			                py = PyPlayer(iPlayer)
			                apUnitList = py.getUnitList()
			                for pUnit in apUnitList:
                                                if pUnit.getUnitClassType() == gc.getInfoTypeForString('UNITCLASS_SETTLER'):
                        	                        newUnit1 = player.initUnit(gc.getInfoTypeForString("UNIT_INFANTRY"), pUnit.getX(), pUnit.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
		#AI infantry bonus on emperor and above
...
## AI AutoPlay ##
		if CyGame().getAIAutoPlay(gc.getGame().getActivePlayer()) == 0:
...
## AI AutoPlay ##
	...
	def onCombatResult(self, argsList):
		...
		# !M! -> Show worm movie when it kills something
		if pWinner.getUnitCombatType() == UnitCombatTypes.WORM_COMBAT:
                        popupInfo = CyPopupInfo()
                        popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
                        popupInfo.setData1(pLoser.getUnitType()) # iType
                        popupInfo.setData2(1) # iMovieEventId
                        popupInfo.setText(u"showMiscMovieScreens")
                        popupInfo.addPopup(pLoser.getOwner())
...
##			popupInfo.addPopup(pCity.getOwner())
                ...
                # Monitor projects
                lMonitorProj = [
                         gc.getInfoTypeForString('PROJECT_MONITOR_ELECTRICAL_SYSTEM'),
                         gc.getInfoTypeForString('PROJECT_MONITOR_WEAPONS_SYSTEM'),
                         gc.getInfoTypeForString('PROJECT_MONITOR_HULL'),
                         gc.getInfoTypeForString('PROJECT_MONITOR_PROPULSION_SYSTEM'),
                         gc.getInfoTypeForString('PROJECT_MONITOR_POWER_SYSTEM')
                ]
                if iProjectType in lMonitorProj:
                        pCapital = pPlayer.getCapitalCity()
                        iMonProjs = 0
			for iProj in lMonitorProj:
				iMonProjs += pTeam.getProjectCount(iProj)
                        if iMonProjs == 4 and not pCapital.isNone():
                                pNew = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_MONITOR_WARSHIP'), pCapital.getX(), pCapital.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
	
		#Ix project
		elif iProjectType == gc.getInfoTypeForString('PROJECT_IX'):
                        for x in range(2):
                                newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_MEDIUM_CARRYALL'), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)
                                newUnit.setName("Ixian No-Carryall")
                                newUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_NO_CLOAK'), True)
                        for jTeam in xrange(gc.getMAX_CIV_TEAMS()):
                                pTeamJ = gc.getTeam(jTeam)
                                if (not pTeamJ.isAlive()) or jTeam == iTeam: continue

                                pTeamJ.setEspionagePointsAgainstTeam(iTeam, 0)

                                for i in xrange(CyMap().numPlots()):
                                        pPlot = CyMap().plotByIndex(i)
                                        if not pPlot.isVisible(jTeam, False):
                                                if pPlot.isRevealed(jTeam, False):
                                                        pPlot.setRevealed(jTeam, False, False, -1)
                        CyMap().updateVisibility()

		#Breeding Projects
                elif iProjectType == gc.getInfoTypeForString('PROJECT_BREEDING1'):
                        pCapital = pPlayer.getCapitalCity()
                        if not pCapital.isNone():
                                pCapital.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH1'), 1)
                                pCapital.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH1'), 1)
		elif iProjectType == gc.getInfoTypeForString('PROJECT_BREEDING2'):
                        (loopCity, iter) = pPlayer.firstCity(False)
			while(loopCity):
                                loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH1'), 0)
                                loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH1'), 0)
                                if loopCity.isCapital():
                                        loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH2'), 1)
                                        loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH2'), 1)
				(loopCity, iter) = pPlayer.nextCity(iter, False)
		elif iProjectType == gc.getInfoTypeForString('PROJECT_BREEDING3'):
                        (loopCity, iter) = pPlayer.firstCity(False)
			while(loopCity):
                                loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH2'), 0)
                                loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH2'), 0)
                                if loopCity.isCapital():
                                        loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH3'), 1)
                                        loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH3'), 1)
				(loopCity, iter) = pPlayer.nextCity(iter, False)
		elif iProjectType == gc.getInfoTypeForString('PROJECT_BREEDING4'):
			pTeam.setHasTech(gc.getInfoTypeForString('TECH_PRESCIENCE2'), True, iPlayer, False, True)
                        (loopCity, iter) = pPlayer.firstCity(False)
			while(loopCity):
                                loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH3'), 0)
                                loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH3'), 0)
                                if loopCity.isCapital():
                                        loopCity.setFreeSpecialistCount(gc.getInfoTypeForString('SPECIALIST_KWIZATZ_HADERACH4'), 1)
                                        loopCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_KH4'), 1)
				(loopCity, iter) = pPlayer.nextCity(iter, False)
		elif iProjectType == gc.getInfoTypeForString('PROJECT_TEG'):
                        pTeg = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_TEG'), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
	...
	def onUnitPillage(self, argsList):
		...
		# !M! -> Show worm movie when it destroys a harvester
		if pUnit.getUnitCombatType() == UnitCombatTypes.WORM_COMBAT:
                        popupInfo = CyPopupInfo()
                        popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
                        popupInfo.setData1(iImprovement) # iType
                        popupInfo.setData2(0) # iMovieEventId
                        popupInfo.setText(u"showMiscMovieScreens")
...
##		pCity = pPlot.getPlotCity()
	...
	def onGoodyReceived(self, argsList):
		...
		# !M! -> Show goody splash when applicable
		if iPlayer > -1 and gc.getPlayer(iPlayer).isHuman():
                        if not CyUserProfile().getGraphicOption(GraphicOptionTypes.GRAPHICOPTION_NO_MOVIES):
                                if CyGame().isFinalInitialized() and not CyGame().GetWorldBuilderMode():
                                        if (not CyGame().isGameMultiPlayer()) and (iPlayer == CyGame().getActivePlayer()):
                                                popupInfo = CyPopupInfo()
                                                popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON_SCREEN)
                                                popupInfo.setData1(iGoodyType) # iType
                                                popupInfo.setData2(0) # iEventId
...
## Growth Victory Start ##
		if CyGame().isVictoryValid(gc.getInfoTypeForString("VICTORY_GROWTH")):
                        iGoal = 29 + gc.getMap().getWorldSize()
                        if pCity.getPopulation() >= iGoal:
                                pPlayer = gc.getPlayer(iNewOwner)
                                iCount = 0
                                (loopCity, iter) = pPlayer.firstCity(False)
                                while(loopCity):
                                        if loopCity.getPopulation() >= iGoal:
                                                iCount += 1
                                        if iCount > 2:
                                                CyGame().setWinner(loopCity.getTeam(), gc.getInfoTypeForString("VICTORY_GROWTH"))
                                                break
                                        (loopCity, iter) = pPlayer.nextCity(iter, False)
## Growth Victory End ##
