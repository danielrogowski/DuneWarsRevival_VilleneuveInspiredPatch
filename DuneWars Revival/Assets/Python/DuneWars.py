...
def getWidgetHelp(argsList):
    ...
    ## Platy WorldBuilder ##
        ...
        if iData1 == 1027:
            ...
            if iData2 == 0:
                ...
                return CyTranslator().getText("TXT_KEY_PEDIA_CATEGORY_UNIT", ()) + " + " + CyTranslator().getText(
                    "TXT_KEY_CONCEPT_CITIES", ())
            elif iData2 == 6:
        ...
        ## City Hover Text ##
            ...
            if CyGame().GetWorldBuilderMode():
                ...
                sText += u"\n%s: %d/%d" % (
                CyTranslator().getText("[ICON_FOOD]", ()), pCity.getFood(), pCity.growthThreshold())
                ...
                if iGPRate > 0 or iProgress > 0:
                    sText += u"\n%s: %d/%d %+d" % (
                    CyTranslator().getText("[ICON_GREATPEOPLE]", ()), iProgress, pPlayer.greatPeopleThreshold(False),
                    iGPRate)

                sText += u"\n%s: %d/%d (%s)" % (
                CyTranslator().getText("[ICON_CULTURE]", ()), pCity.getCulture(iPlayer), pCity.getCultureThreshold(),
                ...
                for i in xrange(CommerceTypes.NUM_COMMERCE_TYPES):
                    ...
                    sTemp = u"%d.%02d%c" % (
                    pCity.getCommerceRate(i), pCity.getCommerceRateTimes100(i) % 100, gc.getCommerceInfo(i).getChar())
                    lTemp.append(sTemp)
                if len(lTemp) > 0:
                    sText += "\n"
                ...
                if iMaintenance != 0:
                    sText += "\n" + CyTranslator().getText("[COLOR_WARNING_TEXT]", ()) + CyTranslator().getText(
                        "INTERFACE_CITY_MAINTENANCE", ()) + " </color>"
                    sText += u"-%d.%02d%c" % (
                    iMaintenance / 100, iMaintenance % 100, gc.getCommerceInfo(CommerceTypes.COMMERCE_GOLD).getChar())
                ...
                if len(lBuildings) > 0:
                    lBuildings.sort()
                    sText += "\n" + CyTranslator().getText("[COLOR_BUILDING_TEXT]", ()) + CyTranslator().getText(
                        "TXT_KEY_PEDIA_CATEGORY_BUILDING", ()) + ": </color>"
                ...
                if len(lWonders) > 0:
                    lWonders.sort()
                    sText += "\n" + CyTranslator().getText("[COLOR_SELECTED_TEXT]", ()) + CyTranslator().getText(
                        "TXT_KEY_CONCEPT_WONDERS", ()) + ": </color>"
    ...
    # !M! ->
        ...
        # Mentat
            ...
            return CyTranslator().getText("TXT_KEY_MENTAT_ASSIGN", ()) + CyGameTextMgr().getPromotionHelp(
                gc.getInfoTypeForString(sMentatQuality), False)
        # Tleilaxu Gholas:
        elif iData1 == 8999:
            ...
            for i in UnitsData[iData2]["Promotions"]:
                sPromos += (CyTranslator().getText("[ICON_SILVER_STAR]", ()) + gc.getPromotionInfo(
                    i).getDescription() + " ")
            ...
            for i in UnitsData[iData2]["Promotions"]:
                iCountPromos += 1
                if iCountPromos > 11:
                    sPromos += CyTranslator().getText("[NEWLINE][NEWLINE][ICON_SILVER_STAR][SPACE]",
                                                      ()) + CyGameTextMgr().getPromotionHelp(i, False)
                    sPromos2 += CyTranslator().getText("[NEWLINE][ICON_SILVER_STAR][SPACE]%s1",
                                                       (gc.getPromotionInfo(i).getDescription(),))
            if iCountPromos < 16:
                return sPromos
            else:
                return sPromos2
            ...
            for iBldg in xrange(gc.getNumBuildingInfos()):
                if gc.getBuildingInfo(iBldg).getReligionType() > -1:
                    if pCity.getNumBuilding(iBldg) > 0:
                        iCountBldg += 1
                        if iCountBldg > 10:
                            sHelp += CyTranslator().getText("[NEWLINE][NEWLINE][ICON_RELIGION][SPACE]",
                                                            ()) + CyGameTextMgr().getBuildingHelp(iBldg, False, False,
                                                                                                  False, None)
                            sHelp2 += CyTranslator().getText("[NEWLINE][ICON_RELIGION][SPACE]%s1",
                                                             (gc.getBuildingInfo(iBldg).getDescription(),))
            if iCountBldg < 16:
                return sHelp
            else:
                return sHelp2
...
def AbominationDo(argsList):
    ...
    Traits = ['TRAIT_AGGRESSIVE', 'TRAIT_CHARISMATIC', 'TRAIT_CREATIVE', 'TRAIT_EXPANSIVE', 'TRAIT_FINANCIAL',
              'TRAIT_INDUSTRIOUS', 'TRAIT_ORGANIZED', 'TRAIT_PHILOSOPHICAL', 'TRAIT_SPIRITUAL']
...
class DuneWars:
    ...
    def onBeginPlayerTurn(self, argsList):
        ...
        if pPlayer.hasTrait(self.iTraitABo):
            if CyGame().getSorenRandNum(100, "") < 3:
                iEvent = CvUtil.findInfoTypeNum(gc.getEventTriggerInfo, gc.getNumEventTriggerInfos(),
                                                'EVENTTRIGGER_TRAIT_ABOMINATION')
    ...
    def onModNetMessage(self, argsList):
        ...
        # Inquisition:
            ...
            if pUnit.movesLeft() > 0:
                ...
                if bPurgeSucess:
                    ...
                    CyInterface().addMessage(iPlayer, True, 15, CyTranslator().getText("TXT_PURGE_SUCCESS", (
                    ReligionInfo.getDescription().capitalize(), pCity.getName(),)), 'AS2D_PLAGUE', 0,
                                             ReligionInfo.getButton(), gc.getInfoTypeForString("COLOR_POSITIVE_TEXT"),
                    ...
                    for iPlay in xrange(gc.getMAX_CIV_PLAYERS()):
                        ...
                        if pCity.isRevealed(pPlay.getTeam(), false):
                            CyInterface().addMessage(iPlay, false, 15,
                                                     CyTranslator().getText("TXT_KEY_MESSAGE_INQUISITION_GLOBAL", (
                                                     ReligionInfo.getDescription().capitalize(), pCity.getName(),)), "",
                                                     0, "", -1, -1, -1, false, false)
                else:
                    CyInterface().addMessage(iPlayer, True, 15, CyTranslator().getText("TXT_PURGE_FAILURE", (
                    ReligionInfo.getDescription().capitalize(), pCity.getName(),)), 'AS2D_SABOTAGE', 0,
                                             ReligionInfo.getButton(), gc.getInfoTypeForString("COLOR_WARNING_TEXT"),
                                             pCity.getX(), pCity.getY(), True, True)
                pUnit.kill(False, -1)
        ##Tleilaxu Gholas:
        ...
        # Mentat
            ...
            pUnit.getGroup().pushMission(MissionTypes.MISSION_FORTIFY, 0, 0, 0, false, false,
    ...
    def TleilaxuGhola(self, iPlayer, iGholaID, iCellsID):
        ...
        pGhola = pPlayer.initUnit(UnitsData[id]["UnitType"], pHatchedTank.getX(), pHatchedTank.getY(),
        ...
        CyInterface().addMessage(iPlayer, True, 20,
                                 CyTranslator().getText("TXT_GHOLA_COMPLETED", (UnitsData[id]["Name"],)),
                                 'AS2D_UNIT_BUILD_UNIT', 2, gc.getUnitInfo(pGhola.getUnitType()).getButton(),
                                 ColorTypes(12), pGhola.getX(), pGhola.getY(), True, True)
    ...
    def MakeCityName(self, pCity):
        ...
        hillsSuffixArray = ["TXT_KEY_HILLS_SUFFIX_01", "TXT_KEY_HILLS_SUFFIX_02", "TXT_KEY_HILLS_SUFFIX_03",
                            "TXT_KEY_HILLS_SUFFIX_04"]
        nonHillsSuffixArray = ["TXT_KEY_NONHILLS_SUFFIX_01", "TXT_KEY_NONHILLS_SUFFIX_02", "TXT_KEY_NONHILLS_SUFFIX_03",
                               "TXT_KEY_NONHILLS_SUFFIX_04", "TXT_KEY_NONHILLS_SUFFIX_05"]
    ...
    def TerraformUp(self, pPlot, dChances):
        ...
        iType = pPlot.getTerrainType()
        if iType == self.iTSalt:
            iNew = self.iTGrab
        elif iType == self.iTGrab:
            iNew = self.iTTPlain
        elif iType == self.iTRug:
            iNew = self.iTRock
            iChance = iChance / 2
        elif iType == self.iTRock:
            iNew = self.iTTPlain
        elif iType == self.iTTPlain:
            iNew = self.iTTGrass
    ...
    def WormAdd(self, iUnowned):
        ...
        if iWormWant > 0:
            ...
            if iEra == 0 or iEra == 1:
                iWormUnit = self.iUWorm1
            elif iEra == 2 or iEra == 3:
                iWormUnit = self.iUWorm2
            elif iEra == 4 or iEra == 5:
                iWormUnit = self.iUWorm3
            elif iEra == 6:
                iWormUnit = self.iUWorm4
        ...
        if self.iUpdateTimer == 12:
            ...
            for lxyt in self.StormList():
                ...
                if iType == 1:
                    ...
                    if CyMap().plot(ix, iy).isWater() or iPlotType == self.iTDunes:
                        iEffect = self.iESandD
                    else:
                        iEffect = self.iESandL
                else:
                    iEffect = self.iESand  # should be rain
    ...
    def StormDamage(self, pPlot, iX, iY):
        ...
        if iPlotImprov > -1:
            ...
            if pPlot.isHills():
                ...
                iChance = 2
            elif pPlot.getTerrainType() == self.iTOcean:
                iChance = 90
            iPlay = pPlot.getOwner()
            ...
            if CyGame().getSorenRandNum(100, "") < iChance:
                ...
                if iPlay > -1:
                    CyInterface().addMessage(iPlay, True, 10, CyTranslator().getText("TXT_KEY_IMPROVEMENT_DESTROYED_ST",
                                                                                     (gc.getImprovementInfo(
                                                                                         iPlotImprov).getDescription(),)),
                                             "AS2D_COMBAT", 3, gc.getImprovementInfo(iPlotImprov).getButton(),
                                             ColorTypes(7), iX, iY, True, True)
                    # !M! -> Show sandstorm image when applicable
                    if gc.getPlayer(iPlay).isHuman() and not CyUserProfile().getGraphicOption(
                            GraphicOptionTypes.GRAPHICOPTION_NO_MOVIES):
        ...
        if iPlotNumUnits > 0:
            ...
            for iUnit in xrange(iPlotNumUnits):
                ...
                if pUnit.baseCombatStr() > 0:
                    ...
                    if pUnit.isDead():
                        CyInterface().addMessage(pUnit.getOwner(), True, 10,
                                                 CyTranslator().getText("TXT_KEY_UNIT_DESTROYED_ST",
                                                                        (pUnit.getName(),)), "AS2D_COMBAT", 3,
                                                 pUnit.getButton(), ColorTypes(7), iX, iY, True, True)
                    else:
                        CyInterface().addMessage(pUnit.getOwner(), True, 10,
                                                 CyTranslator().getText("TXT_KEY_UNIT_DAMAGED_ST", (pUnit.getName(),)),
                                                 "AS2D_COMBAT", 3, pUnit.getButton(), ColorTypes(11), iX, iY, True,
                                                 True)
                else:
                    if pUnit.getUnitCombatType() == self.iUCombatWorker and CyGame().getSorenRandNum(100, "") < 8:
                        ...
                        CyInterface().addMessage(pUnit.getOwner(), True, 10,
                                                 CyTranslator().getText("TXT_KEY_UNIT_IMMOBILE_ST", (pUnit.getName(),)),
                                                 "AS2D_COMBAT", 3, pUnit.getButton(), ColorTypes(11), iX, iY, True,
                                                 True)
                        pUnit.setImmobileTimer(2)  # GameTurn has not ended yet (= 1)
                # !M! -> Show sandstorm image when applicable (once per plot)
                if gc.getPlayer(pUnit.getOwner()).isHuman() and bShowImage and not CyUserProfile().getGraphicOption(
                        GraphicOptionTypes.GRAPHICOPTION_NO_MOVIES):
                    if CyGame().isFinalInitialized():
    ...
    def StormDie(self, cx, cy):
        ...
        if pPlot.isHills():
            ...
            return true
        elif iType == self.iTPCoast:
            return true
        elif iType == self.iTOcean:
            return false
        elif iRnd < 10:
            return true
        return false
